...
class Session:
    ...
    def to_context(self) -> Dict[str, Optional[str]]:
        """Return minimal session metadata for hooks."""
        return {
            "session_id": self.session_id,
            "backend": self.backend,
            "cli_session_id": self.cli_session_id,
...
class SessionManager:
    ...
    def get_or_create_session(self, session_id: str, backend: str) -> Session:
        ...
        with self.lock:
            if session_id in self.sessions:
                ...
                if session.backend != backend:
                    raise ValueError(
                        f"Session '{session_id}' is bound to backend '{session.backend}', cannot reuse with '{backend}'."
                    )
    ...
    def get_session(self, session_id: str, backend: Optional[str] = None) -> Optional[Session]:
        """Get existing session without creating new one."""
        with self.lock:
            session = self.sessions.get(session_id)
            if session and not session.is_expired():
                if backend and session.backend != backend:
                    logger.warning(
                        f"Session '{session_id}' backend mismatch: expected '{backend}', found '{session.backend}'."
                    )
                    return None
                session.touch()
                return session
    ...
    def process_messages(
        self, messages: List[Message], session_id: Optional[str], backend: str
    ) -> Tuple[List[Message], Optional[str], Optional[Session]]:
        """
        Process messages for a request, handling both stateless and session modes.

        Returns:
            Tuple of (all_messages_for_request, actual_session_id_used, session)
        """
        ...
        session = self.get_or_create_session(session_id, backend)
        ...
        return all_messages, session_id, session
